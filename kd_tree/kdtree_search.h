...
class KDTREE_API KdTreeSearch : public Counted
{
public:
	KdTreeSearch();
	...
	virtual void add_point(PointSet::Vertex* v) = 0;
	virtual void add_vertex_set(PointSet* vs) = 0;
	virtual void end() = 0;

	//________________ closest point ____________________________

	// NOTE: *squared* distance is returned
	virtual PointSet::Vertex* find_closest_point(const vec3& p, double& squared_distance) const = 0;
	virtual PointSet::Vertex* find_closest_point(const vec3& p) const = 0;

	//_________________ K-nearest neighbors ____________________

	// NOTE: *squared* distances are returned
	virtual void find_closest_K_points(const vec3& p, unsigned int k, std::vector<PointSet::Vertex*>& neighbors, std::vector<double>& squared_distances) const = 0;
	virtual void find_closest_K_points(const vec3& p, unsigned int k, std::vector<PointSet::Vertex*>& neighbors) const = 0;
	...
	virtual void find_points_in_radius(const vec3& p, double squared_radius, std::vector<PointSet::Vertex*>& neighbors) const = 0;
	virtual void find_points_in_radius(const vec3& p, double squared_radius, std::vector<PointSet::Vertex*>& neighbors, std::vector<double>& squared_distances) const = 0;
};
