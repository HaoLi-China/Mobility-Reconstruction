...
bool MapSerializer_obj::do_write(std::ostream& out, const Map* mesh) const {
	...
	Attribute<TexVertex, int> tex_vertex_id(mesh->tex_vertex_attribute_manager());
...
bool MapSerializer_eobj::do_write(std::ostream& output, const Map* mesh) const {
	...
	Attribute<TexVertex,int> tex_vertex_id(
		mesh->tex_vertex_attribute_manager()
		) ;
	...
	{ FOR_EACH_FACET_CONST(Map, mesh, it) {
		...
		do {
			if(tex_vertex_id[jt-> tex_vertex()] == 0) {
				tex_vertex_id[jt->tex_vertex()] = cur_id ;
				output << "vt " << jt->tex_vertex()->tex_coord() << std::endl ;
				cur_id++ ;
			}
			jt = jt->next() ;
		} while(jt != it->halfedge()) ;
